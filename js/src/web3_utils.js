...
export const PUBLIC_HTTP_WEB3 = new Web3(
  new Web3.providers.HttpProvider(PUBLIC_HTTPS_HOST, {
    reconnect: {
      auto: true,
      delay: 5000, // ms
      maxAttempts: 30,
...
export const HTTP_WEB3 = new Web3(
  new Web3.providers.HttpProvider(NETWORK_HTTPS_HOST, {
    reconnect: {
      auto: true,
      delay: 5000, // ms
      maxAttempts: 30,
...
export const WEBSOCKET_WEB3 = new Web3(
  new Web3.providers.WebsocketProvider(NETWORK_WSS_HOST, {
    reconnect: {
      auto: true,
      delay: 5000, // ms
      maxAttempts: 30,
...
export const OWNER_WALLET =
  OWNER_WEB3.eth.accounts.privateKeyToAccount(OWNER_PRIVATE_KEY);

export const BUY_OPERATOR_WALLET =
  PUBLIC_HTTP_WEB3.eth.accounts.privateKeyToAccount(BUY_OPERATOR_PRIVATE_KEY);

export const SELL_OPERATOR_WALLET =
  PUBLIC_HTTP_WEB3.eth.accounts.privateKeyToAccount(SELL_OPERATOR_PRIVATE_KEY);
...
export async function estimateGas(transaction) {
  ...
  let estimateGas;
  await HTTP_WEB3.eth.estimateGas(tx).then((r) => (estimateGas = r));
  return estimateGas;
}
