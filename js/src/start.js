...
async function calMint(targetAmountIn, targetPath, targetFinance) {
  //获取可能的交易币种类
  let pairs = [];
  let [amounts, reserves] = await getAmountsOut(
    targetAmountIn,
    targetPath,
    targetFinance.factoryAddress,
    targetFinance.hex,
    targetFinance.fee
  );
