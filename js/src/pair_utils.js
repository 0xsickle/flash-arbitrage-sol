...
export function calPairAddress(tokenA, tokenB, factoryAddress, hex) {
  ...
  let abiEncoded1 = HTTP_WEB3.eth.abi.encodeParameters(
    ["address", "address"],
    [token0, token1]
  );
  abiEncoded1 = abiEncoded1.split("0".repeat(24)).join("");
  let salt = HTTP_WEB3.utils.soliditySha3(abiEncoded1);
  let abiEncoded2 = HTTP_WEB3.eth.abi.encodeParameters(
    ["address", "bytes32"],
    [factoryAddress, salt]
  );
  abiEncoded2 = abiEncoded2.split("0".repeat(24)).join("").substr(2);
  let pair =
    "0x" + HTTP_WEB3.utils.soliditySha3("0xff" + abiEncoded2, hex).substr(26);
  return pair;
}
...
export async function getAmountsOut(
  ...
  for (let i = 0; i < path.length - 1; i++) {
    let [reserveIn, reserveOut] = await getReserves(
      path[i],
      path[i + 1],
      factoryAddress,
      hex
...
//tokenA = token1 = WBNB
//tokenB = token0 = CAKE
let pairAddress = calPairAddress(
